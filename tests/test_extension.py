...
class ExampleTest(unittest.TestCase):
    def alpha(self,cur_marker):
        func = (1+ 1e-9)/(cur_marker + 1e-9)
        return func

    def fminmax_indx(self,tree, altitudes, marker):
        """
        ...
        alpha_marker = self.alpha(marker)
    ...
    def test_fminmax_idx(self):
        ...
        py_version = self.fminmax_indx(tree, np.array(altitudes), mo)[1]
        alpha_mo = self.alpha(mo)
        ...
        self.assertTrue(np.allclose(pass_edge_w, altitudes[pass_edges]))
        self.assertTrue(np.allclose(py_version,closest_vertex))
